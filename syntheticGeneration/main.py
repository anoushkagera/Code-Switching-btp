...
# Classes responsible for training and evaluating the machine translation model.
...
def main(params):
    ...
    for _ in range(trainer.epoch, params.max_epoch):
        ...
        # Within the epoch loop, different training steps are performed based on the specified parameters.
        trainer.n_sentences = 0

        while trainer.n_sentences < params.epoch_size:
# First, if the discriminator loss coefficient (lambda_dis) is greater than 0 (params.lambda_dis > 0), it performs discriminator training for the specified number of iterations (params.n_dis).
            ...
            # Then, if the language model loss coefficient (lambda_lm) is greater than 0 (params.lambda_lm > 0), 
            # it performs language model training for the specified number of iterations after each epoch (params.lm_after) for each language in params.langs.
            if params.lambda_lm > 0:
                for _ in range(params.lm_after):
                    for lang in params.langs:
                        trainer.lm_step(lang)

# Next, if the cross-entropy reconstruction coefficient for parallel data (lambda_xe_para) is greater than 0 (params.lambda_xe_para > 0), 
# it performs machine translation training using parallel data for each language pair specified in params.para_directions.
...
# lambda_xe_mono: Used for autoencoder training on monolingual data.
# In this scenario, the model is trained to reconstruct sentences in the same language without relying on a parallel corpus. 
# The reconstruction loss helps the model learn meaningful representations of the language.

# lambda_xe_para: Used for training on parallel data.
# Parallel data consists of aligned sentences in two languages, 
# where each sentence in one language has a corresponding translation in the other language. 
# The model is trained to generate the target sentence given the source sentence.

# lambda_xe_back: Used for training on back-parallel data.
# Back-parallel data is similar to parallel data, 
# but the source and target languages are reversed in some instances.
# This setup helps the model learn bidirectional translation capabilities.

# lambda_xe_otfd: Used for training with on-the-fly back-translation on parallel data.
# On-the-fly back-translation involves generating synthetic parallel data by translating
# sentences from one language to another and back again. This helps improve the model's translation
# quality by exposing it to more diverse training data.

# lambda_xe_otfa: Used for training with on-the-fly back-translation for autoencoding data.
# Similar to on-the-fly back-translation for parallel data, but applied in an autoencoder setting. It helps the model learn better representations of the source language by reconstructing the original sentence from its translated version.