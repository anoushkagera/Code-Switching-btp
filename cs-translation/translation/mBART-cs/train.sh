...
for Item in 'train' 'valid' 'test' 'testph' ;
    do
...
python3 code2_build_vocab.py --corpus-data "$SCRATCH_DIR/preprocessed/*.spm.*" --langs $MBART_LANGS --output "$SCRATCH_DIR/trimmed/dict.txt"

# 4: trim mBART

python3 code3_trim_mbart.py --pre-train-dir "$SCRATCH_DIR/mbart.cc25/" --ft-dict "$SCRATCH_DIR/trimmed/dict.txt" --langs $MBART_LANGS --output "$SCRATCH_DIR/trimmed/model.pt"
